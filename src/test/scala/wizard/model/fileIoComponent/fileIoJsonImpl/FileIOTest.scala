...
class FileIOTest extends AnyWordSpec with Matchers with TimeLimitedTests {
  ...
  "A JSON FileIO" should {
    ...
    "handle loading with missing firstPlayerIdx" in {
      ...
      val pw = new java.io.PrintWriter(new File("minimal.json"))
      pw.write(minimalJson)
      pw.close()

      val (loadedGame, _, _, _, loadedFirstPlayer) = fileIo.load("minimal.json")
      loadedFirstPlayer should be(0)
      
      new File("minimal.json").delete()
    }
    
    "handle null trump card" in {
       val gameNoTrump = Game(players)
       fileIo.save(gameNoTrump, 1, None, 0, 0, "notrump.json")
       val (_, _, loadedTrump, _, _) = fileIo.load("notrump.json")
       loadedTrump should be(None)
       new File("notrump.json").delete()
    }
  }
}
