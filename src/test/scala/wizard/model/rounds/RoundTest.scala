...
class RoundTest extends AnyWordSpec with Matchers {
    "Round" should {
        ...
        "setTrump should set the trump color" in {
            ...
            round.setTrump(Some(Color.Yellow))
            round.trump.get shouldBe Color.Yellow
        }
        ...
        "isOver should return false if any player has cards in hand" in {
            val player1 = PlayerFactory.createPlayer(Some("Player1"), Human)
            player1.addHand(Hand(List(Card(Value.Seven, Color.Red))))
            val player2 = PlayerFactory.createPlayer(Some("Player2"), Human)
        ...
        "determineTrump should set the trump color based on player input" in {
