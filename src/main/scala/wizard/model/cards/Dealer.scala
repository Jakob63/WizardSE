...
object Dealer extends Observable {
    ...
    var allCards: List[Card] = {
        ...
        } buffer += CardFactory.createCard(value, color) // pattern
        buffer.toList
    }
    var index = 0

    def setState(newState: DealerState): Unit = {
        state = newState
    }  
    
    def shuffleCards(): Boolean = {
        state.shuffleCards(this)
    }
    
    def dealCards(cards_amount: Int, excludeCard: Option[Card] = None): Hand = {
        state.dealCards(this, cards_amount, excludeCard)
    }
